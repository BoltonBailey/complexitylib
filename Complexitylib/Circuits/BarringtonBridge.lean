...
- `Complexity.BP.Computes_retarget` — a program representing `f` through one
  ...
  (`5`-cycles are conjugate, and conjugation is `BP.Computes_conj`).
...
/-- **Retargeting.** If `p` represents `f` through a `5`-cycle `σ`, then for any
    ...
    `5`-cycles are conjugate — `isConj_iff_cycleType_eq` — and conjugation of the
    representing permutation is `BP.Computes_conj`.) -/
