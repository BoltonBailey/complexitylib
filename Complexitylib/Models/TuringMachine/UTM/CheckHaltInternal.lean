...
/-- writeAndMove with readBackWrite and idleDir preserves a tape when read ≠ ▷ and head ≥ 1. -/
private theorem tape_idle_preserve (t : Tape) (hns : t.read ≠ Γ.start) (hh : t.head ≥ 1) :
...
/-- HoareTime for rewindWorkTM with rich postcondition preserving frame.
    The frame condition `P` must be stable under: work tape `idx` head → 1
    (cells preserved), other tapes preserved, output cells/head preserved.

    Requires WF conditions on all tapes for idle preservation during rewind. -/
private theorem rewindWorkTM_rich_hoareTime (idx : Fin 4) (B_tape : ℕ)
    {P : Tape → (Fin 4 → Tape) → Tape → Prop}
    (hP_preserved : ∀ (inp : Tape) (work : Fin 4 → Tape) (out : Tape)
  ...
  suffices h_loop : ∀ (h : ℕ) (c : Cfg 4 (rewindWorkTM idx).Q),
  ...
  | zero =>
    ...
    -- Step 1: moveLeft, read ▷ → moveRight, move right
    have hstep1 : ∃ c₁,
    ...
    -- Step 2: moveRight → done (all tapes use idle/readBackWrite)
    ...
    have hstep2 : ∃ c₂,
      ...
      refine ⟨_, rfl, rfl, ?_, ?_, ?_, ?_, ?_⟩
      · dsimp only []
        rw [show (c₁.work idx) = (c₁.work idx) from rfl]
  ...
  | succ h ih =>
    ...
    obtain ⟨c_f, hreach_f, hhalt_f, hh_f, hcells_f, hinp_f, hout_f, hw_f⟩ :=
      ih c₁ hst1 (by rw [hcells1]; exact hcell0_c) (by intro j hj; rw [hcells1]; exact hnostart_c j hj)
        hh1 hinp1 hout1 hw1
