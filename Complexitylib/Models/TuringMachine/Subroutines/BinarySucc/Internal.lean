...
/-- Head-independent canonical binary contents. Unlike `HasBinaryString`,
this predicate permits a carry or rewind cursor anywhere on the tape. -/
private def HasBinaryContent (t : Tape) (bits : List Bool) : Prop :=
  (∀ i, (h : i < bits.length) →
    t.cells (i + 1) = Γ.ofBool (bits[i]'h)) ∧
  ∀ i, bits.length ≤ i → t.cells (i + 1) = Γ.blank

private theorem HasBinaryString.hasBinaryContent {t : Tape} {bits : List Bool}
    (h : t.HasBinaryString bits) : t.HasBinaryContent bits :=
  h.2

private theorem HasBinaryContent.hasBinaryString {t : Tape} {bits : List Bool}
    (h : t.HasBinaryContent bits) (hhead : t.head = 1) :
    t.HasBinaryString bits :=
  ⟨hhead, h⟩

private theorem HasBinaryContent.cells_ne_start {t : Tape} {bits : List Bool}
...
/-- Overwrite one in-range binary cell while retaining canonical contents. -/
private theorem HasBinaryContent.write_set {t : Tape} {bits : List Bool}
...
/-- Writing at the first blank appends one binary cell. -/
private theorem HasBinaryContent.write_append {t : Tape} {bits : List Bool}
...
/-- Boolean writes away from cell zero preserve the left-end marker. -/
private theorem write_move_cell0 {t : Tape} (symbol : Γ) (dir : Dir3)
