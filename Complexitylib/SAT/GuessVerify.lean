...
theorem satVerifyInnerCfg_eq_startedCfg (M : TM k) (z : List Bool)
    ...
    (hpair : c.work (satPairIdx k) =
      (_root_.initTape (z.map Γ.ofBool)).move Dir3.right)
  ...
  | mk state input work output =>
      simp [satVerifyInnerCfg, verifierStartedState, TM.startedCfg, TM.step, hne,
        Tape.read, _root_.initTape] at hstate hpair hout hwork ⊢
      refine ⟨hstate, ?_, ?_, ?_⟩
      · rw [hpair]
        simpa [TM.startedCfg, TM.step, hne, Tape.read, _root_.initTape] using
...
theorem satPair_cells_ne_start_of_initTape_ofBool_move_right (M : TM k)
    ...
    (hpair : c.work (satPairIdx k) =
      (_root_.initTape (bits.map Γ.ofBool)).move Dir3.right) :
...
private theorem tape_eq_initTape_ofBool_move_right_of_head_cells
  ...
  cases t with
  | mk head cells =>
      simp [Tape.move, _root_.initTape] at hhead hcells ⊢
      subst head
      subst cells
      exact ⟨rfl, rfl⟩
...
private theorem satCounter_trace_preserves_started_blank_other_work
  ...
  | succ T ih =>
      ...
      let c1 : Cfg (k + 3) TM.LinearCounterPhase :=
        counterNTM.trace 1 (fun _ => choices ⟨0, by omega⟩) c
      have h1 : c1.work i = (_root_.initTape []).move Dir3.right := by
...
private theorem satCounter_trace_preserves_started_blank_output
  ...
  | succ T ih =>
      ...
      let c1 : Cfg (k + 3) TM.LinearCounterPhase :=
        counterNTM.trace 1 (fun _ => choices ⟨0, by omega⟩) c
      have h1 : c1.output = (_root_.initTape []).move Dir3.right := by
...
private theorem satCounter_init_boundary_started_blank_other_work
    ...
    satBoundaryWork cT.work i = (_root_.initTape []).move Dir3.right := by
  cases T with
  | zero =>
      change satBoundaryWork (fun _ : Fin (k + 3) => _root_.initTape []) i =
        (_root_.initTape []).move Dir3.right
...
private theorem satCounter_init_boundary_started_blank_output
    ...
    let c0 : Cfg (k + 3) TM.LinearCounterPhase :=
      { state := TM.LinearCounterPhase.scan,
        ...
        output := _root_.initTape [] }
    let cT := counterNTM.trace T choices c0
    satBoundaryOutput cT.output = (_root_.initTape []).move Dir3.right := by
  cases T with
  | zero =>
      change satBoundaryOutput (_root_.initTape []) =
        (_root_.initTape []).move Dir3.right
...
theorem satGuessVerify_counter_init_exits (M : TM k) (x : List Bool)
    ...
    ∃ t, ∃ ht : t ≤ TM.inputLengthPlusOneCounterTime x.length,
      ...
      (satBoundaryInput cT.input).cells =
        (_root_.initTape (x.map Γ.ofBool)).cells ∧
      ...
      (∀ i, i ≠ satCounterIdx k →
        satBoundaryWork cT.work i = (_root_.initTape []).move Dir3.right) ∧
  ...
  have hpre :
      (_root_.initTape (x.map Γ.ofBool) = _root_.initTape (x.map Γ.ofBool) ∧
        (fun _ : Fin (k + 3) => _root_.initTape []) (satCounterIdx k) =
          _root_.initTape []) := by
    simp
  obtain ⟨t, ht, hhalt, hpost, hfirst⟩ :=
    hcounter.exists_first_halt_time_with_post
      (inp := _root_.initTape (x.map Γ.ofBool))
      (work := fun _ : Fin (k + 3) => _root_.initTape [])
      (out := _root_.initTape []) hpre counterChoicesB
  ...
  · have hcells := NTM.trace_input_cells counterNTM t counterChoices c0
    ...
    have hcell0 : cT.input.cells 0 = (_root_.initTape (x.map Γ.ofBool)).cells 0 := by
  ...
  · intro j hj
    have hcells := NTM.trace_input_cells counterNTM t counterChoices c0
    have hinv := TM.TapeInvariant.initTape_ofBool x
    have hcell : cT.input.cells j = (_root_.initTape (x.map Γ.ofBool)).cells j := by
  ...
  · have hcells := NTM.trace_input_cells counterNTM t counterChoices c0
    ...
    have hcell : cT.input.cells j = (_root_.initTape (x.map Γ.ofBool)).cells j := by
      simpa [cT, c0] using congrFun hcells j
    show (satBoundaryInput cT.input).cells j = (_root_.initTape (x.map Γ.ofBool)).cells j
    rw [satBoundaryInput, tape_move_cells, hcell]
  · have htrace_head := NTM.trace_input_head_le counterNTM t counterChoices c0
    have hcT_head : cT.input.head ≤ t := by
      have hc0 : c0.input.head = 0 := by simp [c0, _root_.initTape]
...
theorem satGuessVerify_rewindInput_exits_with_frames_exact_input (M : TM k) (B : ℕ)
    ...
    (hpre : inp.cells 0 = Γ.start ∧
      (∀ j, j ≥ 1 → inp.cells j ≠ Γ.start) ∧ inp.head ≤ B)
    (hinput_cells : inp.cells = (_root_.initTape (x.map Γ.ofBool)).cells)
    (hout : out.read ≠ Γ.start ∧ out.head ≥ 1)
    (hwork : ∀ i, (work i).read ≠ Γ.start ∧ (work i).head ≥ 1)
    (choices : Fin (B + 2 + 1) → Bool) :
    ∃ t, ∃ ht : t ≤ B + 2,
...
theorem satGuessVerify_guess_exits (M : TM k) (B : ℕ)
    ...
    ∃ t, ∃ _ht : t ≤ NTM.guessBoundedTime B 0,
      ...
      ∃ y : List Bool, y.length ≤ B ∧
        satBoundaryWork cT.work (satWitnessIdx k) =
          (_root_.initTape (y.map Γ.ofBool)).move Dir3.right := by
...
theorem satGuessVerify_guess_exits_with_frames (M : TM k) (B : ℕ)
    ...
    (hpre : (work (satWitnessIdx k)).hasBinaryPrefix [] ∧
      (work (satWitnessIdx k)).cells 0 = Γ.start ∧
      (work (satCounterIdx k)).hasUnaryCounter B ∧
      work (satPairIdx k) = (_root_.initTape []).move Dir3.right)
    ...
    ∃ t, ∃ _ht : t ≤ NTM.guessBoundedTime B 0,
      ...
      satBoundaryWork cT.work (satWitnessIdx k) =
        (_root_.initTape (y.map Γ.ofBool)).move Dir3.right ∧
      satBoundaryWork cT.work (satPairIdx k) =
        (_root_.initTape []).move Dir3.right ∧
...
theorem satGuessVerify_guess_generates_with_pair_frame (M : TM k) (B : ℕ)
  ...
  have hwitness_cT :
    ...
    change ((guessNTM.trace t (fun i => choicesExit ⟨i.val, by omega⟩) c0).work
      (satWitnessIdx k)) = (_root_.initTape (y.map Γ.ofBool)).move Dir3.right
    rw [hprefix_t]
  ...
  have hpair_trace :
    ...
    simpa [guessNTM, c0] using
      NTM.guessBoundedNTM_trace_preserves_other_work
        ...
        (by simpa [c0] using hpair_read)
  have hpair_cT :
      cT.work (satPairIdx k) = (_root_.initTape []).move Dir3.right := by
    rw [hpair_trace]
    exact hpre.2.2.2
  refine ⟨?_, ?_, ?_⟩
  · simpa [G, guessNTM, guessChoices, cT, choicesExit, c0] using hexit
...
theorem satGuessVerify_guess_generates_with_input_pair_frame (M : TM k) (B : ℕ)
    ...
    (hlen : y.length ≤ B)
    (hinput : inp = (_root_.initTape (x.map Γ.ofBool)).move Dir3.right)
    (hpre : (work (satWitnessIdx k)).hasBinaryPrefix [] ∧
      (work (satWitnessIdx k)).cells 0 = Γ.start ∧
      (work (satCounterIdx k)).hasUnaryCounter B ∧
      work (satPairIdx k) = (_root_.initTape []).move Dir3.right) :
  ...
  rw [hinput_cT, hinput]
  exact satBoundaryInput_stable ((_root_.initTape (x.map Γ.ofBool)).move Dir3.right)
    (by simp [Tape.move, _root_.initTape])
...
theorem satGuessVerify_rewind_then_guess_generates_pair (M : TM k) (B : ℕ)
    ...
    ∃ T, ∃ choices : Fin T → Bool,
      ∃ cPair : Cfg (k + 3) TM.PairBuildPhase,
        ...
        cPair.input = (_root_.initTape (x.map Γ.ofBool)).move Dir3.right ∧
        cPair.work (satWitnessIdx k) =
          (_root_.initTape (y.map Γ.ofBool)).move Dir3.right ∧
        cPair.work (satPairIdx k) = (_root_.initTape []).move Dir3.right ∧
        cPair.output = out ∧
...
theorem satGuessVerify_setup_generates_pair (M : TM k) (x y : List Bool)
    ...
    ∃ T, ∃ choices : Fin T → Bool,
      ∃ cPair : Cfg (k + 3) TM.PairBuildPhase,
        ...
        cPair.work (satWitnessIdx k) =
          (_root_.initTape (y.map Γ.ofBool)).move Dir3.right ∧
        cPair.work (satPairIdx k) = (_root_.initTape []).move Dir3.right ∧
        cPair.output = (_root_.initTape []).move Dir3.right ∧
        (∀ i : Fin k, cPair.work (satVerifierWorkIdx i) =
          (_root_.initTape []).move Dir3.right) := by
  ...
  have hcounter' :
      ...
      (∀ i, i ≠ satCounterIdx k →
        satBoundaryWork cC.work i = (_root_.initTape []).move Dir3.right) ∧
  ...
  have hcounterInputCells :
      inpR.cells = (_root_.initTape (x.map Γ.ofBool)).cells :=
    hcounter'.2.2.2.1
  have hcounterUnary :
  ...
  have hothers :
      ∀ i, i ≠ satCounterIdx k →
        workR i = (_root_.initTape []).move Dir3.right :=
    hcounter'.2.2.2.2.2.2.1
  have hwitnessBlank :
      workR (satWitnessIdx k) = (_root_.initTape []).move Dir3.right :=
    hcounter'.2.2.2.2.2.2.2.1
  have hpairBlank :
      workR (satPairIdx k) = (_root_.initTape []).move Dir3.right :=
    hcounter'.2.2.2.2.2.2.2.2.1
  have houtBlank : outR = (_root_.initTape []).move Dir3.right :=
    hcounter'.2.2.2.2.2.2.2.2.2
  have houtR : outR.read ≠ Γ.start ∧ outR.head ≥ 1 := by
    rw [houtBlank]
    constructor <;> simp [Tape.read, Tape.move, _root_.initTape]
  ...
  have hguessPreR :
      ...
      (workR (satCounterIdx k)).hasUnaryCounter (x.length + 1) ∧
      workR (satPairIdx k) = (_root_.initTape []).move Dir3.right := by
...
theorem satGuessVerify_setup_exits_with_pair_frames (M : TM k) (x : List Bool)
  ...
  have hguessInput :
    ...
    rw [hrewind'.2.1]
    exact ⟨by simp [Tape.move, _root_.initTape],
      initTape_ofBool_move_right_cells_ne_start x⟩
...
theorem satGuessVerify_pair_exits (M : TM k) (x y : List Bool)
    ...
    (hpre :
      work (satWitnessIdx k) = (_root_.initTape (y.map Γ.ofBool)).move Dir3.right ∧
      work (satPairIdx k) = (_root_.initTape []).move Dir3.right)
      ...
      (satGuessVerifyNTM M).trace (t + 1)
        ...
        satVerifyWrap M
          { state := verifierStartedState M,
            ...
            output := satBoundaryOutput cT.output } ∧
      satBoundaryWork cT.work (satPairIdx k) =
        (_root_.initTape ((pair x y).map Γ.ofBool)).move Dir3.right := by
  let P := TM.pairBuildTime x.length y.length
  let pairNTM := (TM.pairBuildTM (satWitnessIdx k) (satPairIdx k)).toNTM
  let pairChoicesP : Fin P → Bool := fun i => choices ⟨i.val, by omega⟩
  let inp0 := (_root_.initTape (x.map Γ.ofBool)).move Dir3.right
  let c0 : Cfg (k + 3) TM.PairBuildPhase :=
  ...
  have hpre' :
      inp0 = (_root_.initTape (x.map Γ.ofBool)).move Dir3.right ∧
      work (satWitnessIdx k) = (_root_.initTape (y.map Γ.ofBool)).move Dir3.right ∧
      work (satPairIdx k) = (_root_.initTape []).move Dir3.right := by
...
theorem satGuessVerify_pair_exits_with_verifier_frames (M : TM k) (x y : List Bool)
    ...
    ∃ t, ∃ ht : t ≤ TM.pairBuildTime x.length y.length,
      ...
      cVerify.work (satPairIdx k) =
        (_root_.initTape ((pair x y).map Γ.ofBool)).move Dir3.right ∧
      cVerify.output = out ∧
...
theorem satGuessVerify_pair_start_halts_of_decidesInTime (M : TM k)
    ...
    (hpre :
      work (satWitnessIdx k) = (_root_.initTape (y.map Γ.ofBool)).move Dir3.right ∧
      work (satPairIdx k) = (_root_.initTape []).move Dir3.right)
    (hout : out = (_root_.initTape []).move Dir3.right)
  ...
  have hpairExact : cVerify.work (satPairIdx k) =
    ...
    simpa [P, pairNTM, pairChoices, c0, cT, cVerify, pairExitChoices] using
      hpairExit.2.1
  have houtputExact : cVerify.output = (_root_.initTape []).move Dir3.right := by
...
theorem satGuessVerify_allPathsHaltIn_of_decidesInTime (M : TM k)
  ...
  have hpairInput :
      cPair.input = (_root_.initTape (x.map Γ.ofBool)).move Dir3.right := hsetup.2.2.1
  have hpairWitness :
      cPair.work (satWitnessIdx k) =
        (_root_.initTape (y.map Γ.ofBool)).move Dir3.right := hsetup.2.2.2.1
  have hpairBlank :
      cPair.work (satPairIdx k) = (_root_.initTape []).move Dir3.right := hsetup.2.2.2.2.1
  have hpairOutput :
      cPair.output = (_root_.initTape []).move Dir3.right := hsetup.2.2.2.2.2.1
  have hpairVerifierWork :
      ∀ i : Fin k, cPair.work (satVerifierWorkIdx i) =
        (_root_.initTape []).move Dir3.right := hsetup.2.2.2.2.2.2
  let cPair0 : Cfg (k + 3) TM.PairBuildPhase :=
    { state := TM.PairBuildPhase.init,
      input := (_root_.initTape (x.map Γ.ofBool)).move Dir3.right,
      work := cPair.work,
