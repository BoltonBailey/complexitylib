...
- `ConstraintGraph.preprocessLoops` — its self-loops
...
noncomputable def preprocessLoops : (G.preprocess E).graph.Loops :=
  ((G.reduce E).expanderize E).graph.addLoopsLoops
