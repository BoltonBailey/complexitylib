...
- `RegCSP.inv_card_dart_le_unsatVal` — an unsatisfiable system has value at
  least one dart's worth
-/
...
theorem card_dart_pos_of_not_satisfiable (h : ¬ R.Satisfiable) [Nonempty α] :
    0 < R.graph.order * R.graph.deg := by
  rcases Nat.eq_zero_or_pos (R.graph.order * R.graph.deg) with h0 | h0
  · refine absurd ⟨(fun _ => Classical.arbitrary α : R.Assignment), fun p => ?_⟩ h
    have hcard : Fintype.card R.Dart = 0 := by rw [card_dart, h0]
    have hempty : IsEmpty R.Dart := Fintype.card_eq_zero_iff.mp hcard
    exact hempty.elim p
  · exact h0
...
/-- On an unsatisfiable graph every assignment fails a dart, so the value is at
least one dart's worth. -/
theorem inv_card_dart_le_unsatVal (h : ¬ R.Satisfiable) :
  ...
  have hne : (R.unsatDarts a).Nonempty := by
    ...
    refine h ⟨a, fun p => ?_⟩
    by_contra hp
    simpa [hcon] using (mem_unsatDarts (R := R) (a := a) (p := p)).mpr hp
  have hcard : (1 : ℚ) ≤ ((R.unsatDarts a).card : ℚ) := by
