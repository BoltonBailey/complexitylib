...
*ruler*, and the verbatim suffix is truncated to its length. This is the width
clamp that makes an iterated `FP` step function run in polynomial time — the
ruler is a string whose length is the intended polynomial bound, so the clamp is
carried around in the data rather than computed.

The machine `takeLenTM` has one work tape and seven control states:
...
- `Complexity.takeLen_pair` — the function truncates to the ruler's length
