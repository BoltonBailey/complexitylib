...
/-!
# Incrementing a fixed-width counter in polynomial time

`bumpBits` adds one to a bit list of fixed width, and `bumpCode` packs the
carry state so that the increment can be run by `Cobham.iterate_mem_FP`. The
value lemmas relate the result to `binValLE`, and `bumpCodeFn_mem_FP` puts the
whole step in `FP`. Used to enumerate coin tosses in order.

## Main definitions

- `addBit`, `bumpBits`, `bumpOver` — the increment and its overflow flag
- `bumpStep`, `bumpPack`, `bumpRun`, `bumpCode` — the packed iteration
- `bumpCodeFn_mem_FP` — the packed step is polynomial-time
-/
